...
function Viewport(data) {
    ...
    bindEvent(document.getElementById('wrapper'), 'mousedown', function () {
        self.down = true;
    });

    bindEvent(document.getElementById('wrapper'), 'mouseup', function () {
        self.down = false;
    });

    bindEvent(document.getElementById('wrapper'), 'keyup', function () {
        self.down = false;
    });

    bindEvent(document.getElementById('wrapper'), 'mousemove', function (e) {
        self.mouseX = e.pageX;
        self.mouseY = e.pageY;
    });

    bindEvent(document.getElementById('wrapper'), 'touchstart', function (e) {
    ...
    bindEvent(document.getElementById('wrapper'), 'touchmove', function (e) {
        if (e.preventDefault) {
            e.preventDefault();
        }
    ...
    bindEvent(document.getElementById('wrapper'), 'touchend', function (e) {
        self.down = false;
    });
...
Cube.prototype.sideChange = function () {
    ...
    for (var i = 0; i < this.sides.length; ++i) {
        const element = this.sides[i].getElementsByClassName('cube-image')[0];
        const correspondingSectionId = `${element.id}-section`;
        ...
        correspondingSection.className = '';
    }

    const element = this.sides[this.viewport.currentSide - 1].getElementsByClassName('cube-image')[0];
    const correspondingSectionId = `${element.id}-section`;
    const correspondingSection = document.getElementById(correspondingSectionId);

    element.className = 'cube-image active';
    correspondingSection.className = 'active';
    correspondingSection.style.borderLeftColor = `2rem solid ${element.style.backgroundColor}`;
}
